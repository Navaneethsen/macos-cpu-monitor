...
if [ -f "$CONFIG_FILE" ]; then
    ...
    PERCENTAGE=$(python3 -c "import json; print(json.load(open('$CONFIG_FILE'))['percentage'])" 2>/dev/null || echo "10")
else
...
if launchctl list | grep -q "$SERVICE_NAME"; then
    ...
    echo "   - Alerts trigger when ${PERCENTAGE}% of readings > ${CPU_THRESHOLD}% over ${MONITORING_WINDOW} minutes"
    echo ""
